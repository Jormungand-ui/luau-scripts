...
-- Vrai chemin MM2 (scanné en direct) :
-- Workspace.[NomMap].CoinContainer.Coin_Server [Part]
-- La map change à chaque round donc on cherche CoinContainer dans tous les enfants du Workspace
local function trouverPieces()
    local pieces = {}
    pcall(function()
        for _, mapObj in ipairs(workspace:GetChildren()) do
            -- Chercher CoinContainer dans chaque objet du Workspace (peu importe la map)
            local coinContainer = mapObj:FindFirstChild("CoinContainer", true)
            if coinContainer then
                for _, obj in ipairs(coinContainer:GetDescendants()) do
                    -- La pièce collectible est Coin_Server (Part)
        ...
        if #pieces == 0 then
            print("[MM2] Aucune pièce trouvée — round pas encore commencé ?")
        else
            print("[MM2] " .. #pieces .. " pièce(s) trouvée(s)")
        end
    end)
    return pieces
end

-- ─── Déplacement en Tween (anti-kick) ─────────────────
...
local function deplacerVersPiece(part)
    ...
    pcall(function()
        ...
        -- Vitesse adaptée à la distance (plus naturel)
        -- Max 50 studs/sec pour rester crédible
        local duree = math.clamp(distance / 50, 0.1, 1.5)
        ...
        tween:Play()
        tween.Completed:Wait() -- Attendre la fin du tween avant de continuer
        succes = true
    end)
    return succes
end
...
-- ─── Boucle Farm ───────────────────────────────────────
-- Boucle séquentielle simple : une pièce à la fois, pas de Heartbeat
local compteur = 0

task.spawn(function()
    while true do
        -- Si farm désactivé, attendre sans rien faire
        ...
        pcall(function()
            ...
            if #pieces == 0 then
                print("[MM2] En attente des pièces...")
                task.wait(3)
                return
            end
            ...
            for _, piece in ipairs(pieces) do
                ...
                -- 4. Pause entre chaque pièce (anti-kick)
                task.wait(1)
            end
        end)
